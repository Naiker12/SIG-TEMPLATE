...
export async function convertPdfToWord(files: File[]): Promise<ConversionResult> {
  const formData = new FormData();
  files.forEach(file => formData.append("files", file));
  
  const endpoint = `${API_BASE_URL}/files/convert-to-word`;
  ...
  try {
    ...
    const blob = await res.blob();
    const contentDisposition = res.headers.get('content-disposition');
    let filename = files.length > 1 ? "archivos_convertidos.zip" : "archivo.docx";
    ...
    console.error("Failed to fetch from PDF to Word conversion service:", error);
    if (error instanceof Error) {
       throw new Error(`Error al conectar con el servicio de conversión: ${error.message}`);
    }
    throw new Error("Ocurrió un error desconocido al convertir los archivos.");
